<script lang="ts">
  ...
  import Button, {
    ButtonAlignment,
    ButtonVariant,
  } from "$src/lib/controls/Button.svelte";
  import Checkbox from "$src/lib/controls/Checkbox.svelte";
  import { invoke } from "@tauri-apps/api/tauri";
  import { get, readFile } from "$src/lib/http";
  ...
  async function create() {
    readFile(
      "C:/Users/Fritz/Desktop/Berufsschule/IT/projekt/fda_companion/src-tauri/customers.json"
    )
      .then((customers) => {
        //console.log(data)
...
<template>
  <div id="customer_create">
    <h1 class="text heading col-span-3">Read Customer</h1>
    <div class="col-span-3 hbox">
      <PictureInput
        bind:value={input.image}
        css="flex-initial self-start pt-5"
      />
      <div class="flex-1 vbox">
        ...
        />
        <TextInput bind:value={input.lname} name="last_name" />
        <TextInput
    ...
    <TextInput bind:value={input.street} name="street" css="col-span-2" />
    <TextInput bind:value={input.house_number} name="house_number" alignRight />
    <TextInput
      bind:value={input.postal_code}
      type="number"
      name="postal_code"
    />
    <TextInput bind:value={input.city} name="city" />
    <SelectCountry
      bind:value={country}
      on:change={(e) => (input.country_iso3 = e.detail.item.iso3)}
    />
    <Checkbox icon={Icons.Home} bind:value={acceptTerms} css="col-span-2">
      <p class="text">I accept the</p>
      <a class="link" href="https://google.com" target="_BLANK"
